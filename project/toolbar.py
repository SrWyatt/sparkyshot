...
class EditorToolbar(QWidget):
    ...
    def initUI(self):
        ...
        self.setStyleSheet("""
            ...
            QLabel { color: #aaa; font-weight: bold; background: transparent; }
            QSlider::groove:horizontal {
                height: 4px; background: #333; border-radius: 2px;
            }
            QSlider::handle:horizontal {
                background: #888; width: 14px; margin: -5px 0; border-radius: 7px;
            }
        ...
        self.btn_logo.setFixedSize(42, 42)
        ...
        btn_poly = self.add_btn(layout, "tool_polygon.svg", "polygon", "Polygon (Right-click for sides)")
        ...
        self.slider_zoom.setStyleSheet("background: transparent;")
    ...
    def update_active_tool(self, mode):
        ...
        for btn in all_widgets:
            if btn == self.btn_logo: continue

            is_tool = False
            for k, v in self.tool_buttons.items():
                if v == btn:
                    is_tool = True
                    break
            ...
            if is_tool:
                ...
                btn.setStyleSheet(style_action)

    def add_separator(self, layout):
        line = QFrame()
        line.setFrameShape(QFrame.Shape.VLine)
        line.setStyleSheet("background-color: #333; border: none;")
    ...
    def ask_sides(self):
        dlg = SliderDialog("Polygon Sides", self.current_sides, 3, 20, self)
        dlg.move(QCursor.pos())
        if dlg.exec():
            val = dlg.get_value()
            self.current_sides = val
            self.sides_signal.emit(val)
